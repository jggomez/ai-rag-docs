...
def document_cleaner(raw_text):
    ...
    body_match = re.search(r"(?i)Estimados señores:\s*(.*?)(?=\n\s*Atentamente,)", raw_text, re.DOTALL)
    ...
    # Remove repetitive header blocks (Address, NIT, Consortium)
    noise_patterns = [
        r"Calle 7 Sur No\. 42-70.*",
        r"TEL: \d+",
        r"NIT\.? \d+\.\d+\.\d+-\d+",
        r"MEDELLIN[- ]COLOMBIA",
        r"CONSORCIO\s+EINGETEC\s+/\s+SEDIC",
    ...
    # Remove digital signature residuals (Timestamp and IDs)
    body = re.sub(r"\d{10,}\s\d{2}:\d+", "", body) # Signature timestamps
    body = re.sub(r"\b[a-zA-Z0-9]{5,}-[a-zA-Z0-9]{4,}\b", "", body) # Signature IDs
